...
class KANLinear(nn.Module):
    ...
    def __init__(
        ...
        retain_explicit_layernorm: bool = True,
    ) -> None:
        super().__init__()
        ...
        if input_bounding not in {
            "internal_tanh", "prebounded", "legacy_prebounded", "none"
        }:
            raise ValueError(
                "input_bounding must be internal_tanh, prebounded, "
                "legacy_prebounded, or none"
            )
        ...
        self.retain_explicit_layernorm = bool(retain_explicit_layernorm)
        ...
        # Historical explicit-scaling checkpoints serialized an unused
        # LayerNorm.  Retain that layout by default for compatibility, while
        # new explicitly-scaled models can opt out entirely rather than carry
        # dormant normalization parameters.
        self.normalization = (
            nn.LayerNorm(in_features)
            if normalization == "legacy_layernorm" or self.retain_explicit_layernorm
            else nn.Identity()
        )
    ...
    def b_spline_basis(
        self, bounded_inputs: torch.Tensor, *, clamp_inputs: bool = True
    ) -> torch.Tensor:
        """Evaluate the fixed B-spline basis.

        ``prebounded`` callers have already performed the sole robust bounded
        mapping upstream.  They deliberately bypass another clamp here; the
        upstream tanh produces values strictly inside the open knot interval.
        """

        x = (
            bounded_inputs.clamp(-1.0, 1.0 - torch.finfo(bounded_inputs.dtype).eps)
            if clamp_inputs
            else bounded_inputs
        )
    ...
    def effective_spline_coefficients(self) -> torch.Tensor:
        """Spline coefficients after the scale that is used in the forward pass.

        Regularization of a learned spline scale must act on this tensor, not
        on the raw coefficients alone; otherwise the optimizer can evade a
        raw-coefficient penalty by changing the scale.
        """

        coefficients = self.spline_coefficients
        if hasattr(self, "spline_scale"):
            return coefficients * self.spline_scale.unsqueeze(-1)
        return coefficients
    ...
    def forward_with_contributions(self, inputs: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        bounded = (
            ...
            if self.input_bounding == "legacy_prebounded"
            else normalized
        )
        ...
        with torch.autocast(device_type=inputs.device.type, enabled=False):
            basis = self.b_spline_basis(
                bounded.float(), clamp_inputs=self.input_bounding != "prebounded"
            )
