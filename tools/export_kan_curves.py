        ...
        for head in range(model.graph.heads):
            for feature, name in enumerate(model.graph.edge_feature_names):
                ...
                for i in range(args.points):
                    rows.append({"head": head, "feature": name, "feature_index": feature, "standardized_input": float(xs[i]), "physical_input": float(centers[feature] + scales[feature] * xs[i]), "base_contribution": float(base[i]), "spline_contribution": float(spline[i]), "total_contribution": float(total[i]), "first_difference": float(first[i]), "second_difference": float(second[i])})
